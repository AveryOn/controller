...
class ChapterService {
  ...
  async deleteOneByPathName(pathName) {
    ...
    const chapter = await this.findByPathName(pathName, { select: ["id"] });
    if (chapter) {
      await this.instanceDb.run(`
                DELETE FROM blocks 
                WHERE chapter_id = ?;
...
class SubChapterService {
  ...
  async deleteOneByFullpath(fullpath) {
    ...
    const subChapter = await this.findByFullpath(fullpath, { select: ["id"] });
    if (subChapter) {
      await this.instanceDb.run(`
                DELETE FROM blocks 
...
class BlocksService {
  ...
  async getAllForChapter(params, config2) {
    if (!params) throw new Error("[BlocksService.getAllForChapter]>> params is not defined");
    if (typeof params.chapterId !== "undefined" && (typeof params.chapterId !== "number" || Object.is(+params.chapterId, NaN))) {
  ...
  async getAllForSubChapter(params, config2) {
    if (!params) throw new Error("[BlocksService.getAllForChapter]>> params is not defined");
    if (typeof params.chapterId !== "undefined" && (typeof params.chapterId !== "number" || Object.is(+params.chapterId, NaN))) {
      throw new Error("[BlocksService.getAllForChapter]>> invalid chapterId");
    }
